...
執行：
streamlit run tdcc_trend_app_v5_2.py
"""
...
CACHE_DIR = Path("tdcc_cache_v5_2")
...
def fetch_one_date(driver, stock_id: str, date_value: str, date_ts: pd.Timestamp) -> pd.DataFrame:
    ...
    WebDriverWait(driver, 20).until(lambda d: len(pd.read_html(d.page_source)) > 0)
...
def app() -> None:
    ...
    st.title("TDCC 集保戶股權分散趨勢查詢 v5.2")
